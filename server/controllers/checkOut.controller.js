...
const {
  findCartById,
  getMiniCartById,
} = require('../models/reposities/cart.repo');
...
class CheckoutController {
  ...
  checkOut = async (req, res, next) => {
    ...
    if (!req.userid) {
      ...
      if (userCart.cartOwner.toString() != userInfo._id.toString())
        throw new BadRequestError('This is not your cart');
      if (userCart.cartData.length <= 0)
        throw new BadRequestError('Your cart is empty');

      checkOutOrder = await CheckoutService.CheckOut(
        userInfo,
        userCart,
        orderPayment,
        { receiverInfo, voucherCode }
      );
        ...
        items: [
          {
            ...
            quantity: 2,
            image:
              'https://storage.googleapis.com/luxeperfume/images/product_images/marc-antoine-barrois/ganymede/photo_1.png',
          },
          {
            name: 'Ganymede',
            price: 215,
